        ...
        if (tipo === "text") {
            ...
            data = {
                ...
                template: {
                    ...
                    components: [{ type: "body", parameters: [{ type: "text", text: nome }, { type: "text", text: escola }] }]
                }
            };
        }
...
app.post('/disparar-template', async (req, res) => {
    ...
    if (!telefone || !nome_formando || !projeto_id) return res.status(400).send({ error: "Dados incompletos" });
    try {
        const numero = normalizarNumero(telefone);
        await enviarMensagemMeta(numero, { criança: nome_formando, escola }, "template", usuario || "Evanio");
        await set(ref(db, `vinculo_projeto/${numero}`), { projeto_id, nome: nome_formando, escola, usuario: usuario || "Evanio", data: Date.now() });
        res.send({ ok: true });
    } catch (e) {
...
// ===============================
// 🤖 PROCESSAR MENSAGEM (IA INCORPORADA)
// ===============================
